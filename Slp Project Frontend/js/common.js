...
// FIXED: Use consistent localStorage key - 'amazon_clone_cart'
function getCart() {
    const raw = localStorage.getItem('shopWave_X_cart');
    return raw ? JSON.parse(raw) : {};
}

function saveCart(cart) {
    localStorage.setItem('shopWave_X_cart', JSON.stringify(cart));
    updateCartCount();
}
...
// FIXED: Initialize cart count immediately and multiple times
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    updateCartCount(); // One more time for safety
